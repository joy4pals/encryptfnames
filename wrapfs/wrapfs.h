...
#define DBGRET(x) printk(KERN_DEFAULT "DBG:%s:%s:%d :: ret val:%d\n",	\
__FILE__, __func__, __LINE__, x)
/* #define for constant options and  Variable to store the debug options*/
#define S_DOPS		0x01
#define I_DOPS		0x02
#define D_DOPS		0x04
#define F_DOPS		0x10
#define A_DOPS		0x20
#define ALL_DOPS	0x40

extern int debug_opt;
#endif
extern int wrapfs_mmap_opt;

/* operations vectors defined in specific files */
extern const struct file_operations wrapfs_main_fops;
extern const struct file_operations wrapfs_main_fops_add_space;
...
wrapfs_dentry_to_lower(struct dentry *dentry)
{
	return ((struct wrapfs_dentry_info *)dentry->d_fsdata)->
	lower_path.dentry;
}
